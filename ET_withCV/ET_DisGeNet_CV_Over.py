        ...
        for s in range(len(grid[keys_list[2]])):
            ...
            for m in range(len(grid[keys_list[3]])):
                ...
                combination = {'n_estimators':ne,
                               ...
                               'min_samples_leaf':ml,
                               'max_depth': md}
                metrics_fscore_Over[c], metrics_roc_Over[c] = Cross_Val_Groups(model, X_train, y_train, combination, n_splits = 10, balance = 'over')
...
keys_list = list(metrics_fscore_Over)
fscore_train_mean = []
fscore_train_std = []
fscore_val_mean = []
fscore_val_std = []
...
for i in range(len(keys_list)):
    fscore_val_mean.append(metrics_fscore_Over[keys_list[i]]['mean_fscore_val'])
    fscore_val_std.append(metrics_fscore_Over[keys_list[i]]['std_fscore_val'])
    fscore_train_mean.append(metrics_fscore_Over[keys_list[i]]['mean_fscore_train'])
    fscore_train_std.append(metrics_fscore_Over[keys_list[i]]['std_fscore_train'])
...
fscore_df.to_csv(os.path.join(outDir, 'ET.DisGeNet.fscore.Over.CV.txt'), index=None, sep='\t')
...
######################################
keys_list = list(metrics_roc_Over)
roc_val_mean = []
roc_val_std = []
roc_train_mean = []
roc_train_std=[]
hyperparam = []
for i in range(len(keys_list)):
    roc_val_mean.append(metrics_roc_Over[keys_list[i]]['mean_roc_val'])
    roc_val_std.append(metrics_roc_Over[keys_list[i]]['std_roc_val'])
    roc_train_mean.append(metrics_roc_Over[keys_list[i]]['mean_roc_train'])
    roc_train_std.append(metrics_roc_Over[keys_list[i]]['std_roc_train'])
    hyperparam.append('|'.join(str(p) for p in keys_list[i]))
...
roc_df.to_csv(os.path.join(outDir, 'ET.DisGeNet.roc.Over.CV.txt'), index=None, sep='\t')
