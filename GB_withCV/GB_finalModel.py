...
#1_SelectBestParameters_GB.R
#100|0.0001|0.7|9|deviance
modTree = GradientBoostingClassifier(n_estimators=100,
...
PredictVars.to_csv(os.path.join(outDir, 'GB.DisGeNet.Predictors.CV.txt'), index=None, sep='\t')

#Calculate accuracy
yhat=modTree.predict(x_test)
print('Accuracy is:',metrics.accuracy_score(y_test, yhat))#Accuracy is: 0.8007246376811594

#Calculate ROC AUC
mpred = modTree.predict_proba(x_test)
pred=mpred[:,1]
fpr, tpr, thresholds = metrics.roc_curve(y_test, pred, pos_label=1)
print('ROC AUC is:',metrics.auc(fpr, tpr))#ROC AUC is: 0.9117307288384793

#Calculate the fbeta score
print('f-beta is:', metrics.fbeta_score(y_test, yhat, average='binary', beta=1))#f-beta is: 0.7999999999999999
...
print('Sensitivity is:',TPR)#Sensitivity is: 0.7971014492753623
print('Specificity is:',TNR)#Specificity is: 0.8043478260869565
print('Positive Predicted Value is:',PPV)#Positive Predicted Value is: 0.8029197080291971
print('Negative Predicted Value is:',NPV)#Negative Predicted Value is: 0.7985611510791367
...
x_telab.to_csv(os.path.join(outDir, 'GB.DisGeNet.Samples.CV.txt'), index=None, sep='\t')
