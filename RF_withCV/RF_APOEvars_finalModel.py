...
########################################################################################
a_file = open(os.path.join(outDir, 'RF.APOEvars.ytestFinal.txt'), "w")
np.savetxt(a_file, y_test)
a_file.close()

a_file = open(os.path.join(outDir, 'RF.APOEvars.ytrainFinal.txt'), "w")
...
#1_SelectBestParameters_RF.R
#60|5|2|7
modTree = RandomForestClassifier(n_estimators=60,
                               min_samples_split=5,
                               min_samples_leaf= 2,
                               max_depth=7,
                               random_state=0)
...
PredictVars.to_csv(os.path.join(outDir, 'RF.APOEvars.Predictors.CV.txt'), index=None, sep='\t')

#Calculate accuracy
yhat=modTree.predict(x_test)
print('Accuracy is:',metrics.accuracy_score(y_test, yhat))#Accuracy is: 0.7391304347826086

#Calculate ROC AUC
mpred = modTree.predict_proba(x_test)
pred=mpred[:,1]
fpr, tpr, thresholds = metrics.roc_curve(y_test, pred, pos_label=1)
print('ROC AUC is:',metrics.auc(fpr, tpr))#ROC AUC is: 0.8043478260869565

#Calculate the fbeta score
print('f-beta is:', metrics.fbeta_score(y_test, yhat, average='binary', beta=1))#f-beta is: 0.7352941176470589
...
print('Sensitivity is:',TPR)#Sensitivity is: 0.7246376811594203
print('Specificity is:',TNR)#Specificity is: 0.7536231884057971
print('Positive Predicted Value is:',PPV)#Positive Predicted Value is: 0.746268656716418
print('Negative Predicted Value is:',NPV)#Negative Predicted Value is: 0.7323943661971831
...
x_telab.to_csv(os.path.join(outDir, 'RF.APOEvars.Samples.CV.txt'), index=None, sep='\t')
